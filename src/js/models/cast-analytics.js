  ...
  startTracking() {
    this.castEventTypes.forEach((eventType) => {
      if (eventType.owner == EventOwner.PLAYER_MANAGER) {
        ...
        console.error(
          "Unrecognized CastEventType: " + JSON.stringify(eventType)
        );
      }
    });
  }
...
class SenderTracker extends Tracker {
  constructor() {
    ...
    this.SenderIdRegex = {
      ...
      VOICE: "^__inject__$",
      CHROME_SENDER:
        "^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-" +
        "[a-z0-9]{12}.[0-9]+:[0-9]+$",
      IOS_SENDER:
        "^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-" +
...
class ContentTracker extends Tracker {
  constructor() {
    super("content");
    this.castEventTypes = [CastEventType.PLAYER_LOAD_COMPLETE];
  }
  ...
  handleEvent(event) {
    ...
    data.id =
      event.media.entity || event.media.contentId || event.media.contentUrl;
    this.sendData(data);
  }
